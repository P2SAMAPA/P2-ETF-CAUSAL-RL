...
def train(
    ...
    print(f"\nFitting causal graph on {train_data.shape[0]} train days...")
    graph = fit_causal_graph(train_data, var_names)
    print(f"Graph: {graph.n_edges} edges, method={graph.method}")
    ...
    api.upload_file(
        path_or_fileobj=io.BytesIO(graph_bytes),
        path_in_repo=config.CKPT_GRAPH.format(slug=slug),
        repo_id=config.HF_MODEL_REPO, repo_type="model",
        commit_message=f"Causal graph {slug} — {config.TODAY}",
    )
